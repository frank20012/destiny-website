...
const renderCounts = () => {
  ...
  allAdminTransactions.forEach((transaction) => {
    ...
    if (transaction.type === "refund" && transaction.status === "completed") {
      refundTotal += Number(transaction.amount);
    }
  });
...
const renderAdminTransactionsPage = () => {
  ...
  currentTransactions.forEach((transaction) => {
    ...
    tr.innerHTML = `
      ...
      <td>${transaction.user ? `${transaction.user.firstName} ${transaction.user.lastName}` : "-"}</td>
...
const fetchAdminTransactions = async () => {
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/api/transactions/admin/all`, {
      headers: {
